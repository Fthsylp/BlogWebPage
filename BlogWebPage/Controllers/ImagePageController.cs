...
namespace BlogWebPage.Controllers
{
    ...
    {
        // GET: ImagePage
        readonly GenericRepository<Images> repo = new GenericRepository<Images>();
        ...
        [HttpPost]
        public ActionResult AddPost(Images p)
        {
            if (Request.Files.Count > 0)
            {
                ...
                p.CoverImage = "/Image/" + filename + extension;
            }
            p.Date = DateTime.Now;
            repo.Add(p);
        ...
        [HttpPost]
        public ActionResult EditPost(Images t)
        {
            ...
            if (Request.Files.Count > 0 && !string.IsNullOrEmpty(Request.Files[0].FileName)) //&& !string.IsNullOrEmpty(Request.Files[0].FileName this code checks whether new img has added or not.
            {
                ...
                string ImgFile = "~/Image/" + filename + extension;
                Request.Files[0].SaveAs(Server.MapPath(ImgFile));
                value.CoverImage = "/Image/" + filename + extension;
            }
            if (t.Date.HasValue)
            {
